...
- **上下文压缩**：优先使用服务商返回的真实 token usage，缺失时按字符估算；达到 90% 时让模型压缩早前对话中的时间、地点、事件与约定，保留最早和最近各两条对话作为参考，角色卡与系统提示词保持不变。
...
Android 测试覆盖角色上下文结构、记忆预算、上下文压缩、TTS 竞态去重、多供应商 TTS 适配、直连请求和 Key 不落 `localStorage`。
